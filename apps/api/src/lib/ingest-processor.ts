...
export async function processEmailInline(params: ProcessEmailParams): Promise<ProcessEmailResult> {
  ...
  logger.info(`Processing email inline — ingestJob=${ingestJob.id} document=${document.id}`);
  ...
  try {
    ...
    errors.push(message);
    logger.error(`IngestPipeline failed for job ${ingestJob.id}:`, message);
  }
  ...
  if (extraction) {
    ...
    // Step 7: Create TripEvent records if tripId is provided
    ...
    // Step 6: Set Document status to FAILED if extraction failed
    try {
      await prisma.document.update({
        where: { id: document.id },
        data: { status: "FAILED" },
  ...
  // Step 8: Update IngestJob status
